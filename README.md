...
## Overview
As a challenging task of high-level video understanding, weakly supervised temporal action localization has been attracting increasing attention. Because of the absence of frame-wise annotations, existing methods seek to handle this task with a localization-by-classification framework, which generally adopts a selector to select snippets of high probabilities of actions or namely foreground. However, the existing foreground selection strategies have a major limitation of only considering the unilateral relation from foreground to actions, which cannot guarantee the foreground-action consistency. In this paper, we present a novel framework named FAC-Net to address this issue. Based on the I3D backbone, our method appends three branches on top of it. First, a novel class-wise foreground classification branch is proposed to regularize the relation between actions and foreground, and a class-agnostic attention branch and a multiple instance learning branch are adopted to regularize the foreground-action consistency and help to learn a meaningful foreground feature. Within each branch, we introduce a hybrid attention mechanism, which calculates multiple attention scores for each snippet, to focus on both discriminative and less-discriminative snippets to capture the full action boundaries. Comprehensive experiments on two benchmarks, THUMOS14 and ActivityNet1.3, demonstrate the effectiveness of the proposed method and superior performance over state-of-the-art approaches.
